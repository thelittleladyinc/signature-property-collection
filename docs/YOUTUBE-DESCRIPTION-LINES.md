# Paste-ready description lines for your existing videos

Generated from `build/data/local_spots.json`, so every URL and view count here is
real. Regenerate with `python3 build/tools/description_lines.py`.

## Why this is worth an evening

These 24 videos have **18,735 views** between them, and your
reviews add another **15,350**. That audience already exists and already
trusts your taste in local places. Right now most of those descriptions point at the
old site or nowhere useful, so the traffic evaporates. One line per video sends it
somewhere you own — a page listing the other places you've covered in that town, and
the homes for sale near them.

Do the top five and you have covered most of the reach.

---

## The videos, most-watched first

### 1,820 views — Why Everyone Loves Living in Erie Colorado!
`youtube.com/watch?v=JFfx8G9OxP0`  ·  covers: 24 Carrot Bistro, Erie

Add to the description (near the top, above the hashtags):

```
📍 More of Erie the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/erie.html
```

### 1,635 views — #elpueblito loveland restaurants
`youtube.com/watch?v=0_6C5TnyQLA`  ·  covers: El Pueblito Mexican Restaurant

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 1,571 views — Red Feather Lakes: The Hidden Gem of Colorado
`youtube.com/watch?v=_ich5kS-VUY`  ·  covers: Red Feather Lakes

Add to the description (near the top, above the hashtags):

```
📍 More of Red Feather Lakes the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/red-feather-lakes.html
```

### 1,439 views — What Makes Loveland's Cherry Treats So Special?
`youtube.com/watch?v=Irv3SlJK9KY`  ·  covers: The Colorado Cherry Company

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 1,322 views — Is This the Best Steakhouse in Loveland?
`youtube.com/watch?v=6EbyMIobZ1Y`  ·  covers: The Loveland Chophouse

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 1,314 views — Trying Sushi at Mr. Chen for the First Time! Would I eat here again?
`youtube.com/watch?v=gZn9cSIcTpo`  ·  covers: Mr. Chen Sushi & Hibachi Steak House

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 1,188 views — Sweetheart Winery is one reason I moved back to Loveland
`youtube.com/watch?v=nqPzw2QUjzA`  ·  covers: Sweet Heart Winery

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 1,107 views — Riding Through the Historic Baldwin Tunnel – Poudre Canyon, Colorado
`youtube.com/watch?v=Lv1K3yQaPWo`  ·  covers: Poudre Canyon & the Baldwin Tunnel

Add to the description (near the top, above the hashtags):

```
📍 More of Bellvue the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/fort-collins.html
```

### 1,096 views — Is This the Cutest Home in Windsor, Colorado?
`youtube.com/watch?v=SAZceZQJrAs`  ·  covers: Windsor Lake & Boardwalk Park

Add to the description (near the top, above the hashtags):

```
📍 More of Windsor the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/windsor.html
```

### 996 views — Downtown Loveland — Sweetheart City's Best Events!
`youtube.com/watch?v=OGoRYV2bGA8`  ·  covers: Downtown Loveland, Sage

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 770 views — If you're planning to hike Devil's Backbone, here are 3 things you NEED to know
`youtube.com/watch?v=2mr0--sAM7s`  ·  covers: Devil's Backbone Open Space

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 744 views — Upgrade Your View: Luxurious Living in Windsor, Colorado
`youtube.com/watch?v=K8sjM8_7o5I`  ·  covers: Windsor Mill Tavern

Add to the description (near the top, above the hashtags):

```
📍 More of Windsor the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/windsor.html
```

### 557 views — Inside the Eaton Country Club with Mccarty's: A Hidden Gem
`youtube.com/watch?v=zTisKr7I1ZA`  ·  covers: Eaton Country Club

Add to the description (near the top, above the hashtags):

```
📍 More of Eaton the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/eaton.html
```

### 553 views — Unveiling Eaton, Colorado: Hidden Gem with Gelato, Fun & Family
`youtube.com/watch?v=dZfHrceqlw4`  ·  covers: Eaton Recreation Center, Route 85 Grill

Add to the description (near the top, above the hashtags):

```
📍 More of Eaton the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/eaton.html
```

### 551 views — Why Erie, Colorado Feels Like Home | Scenic, Social, & So Worth It
`youtube.com/watch?v=xJvhWG2E9qw`  ·  covers: Birdhouse, Colorado National Golf Club

Add to the description (near the top, above the hashtags):

```
📍 More of Erie the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/erie.html
```

### 466 views — Venezia: Eaton's Best Italian Restaurant
`youtube.com/watch?v=BQswNwyMDG8`  ·  covers: Venezia Italian Restaurant & Bar

Add to the description (near the top, above the hashtags):

```
📍 More of Eaton the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/eaton.html
```

### 445 views — Discover Ault: Hidden Gems and Local Dining Spots!
`youtube.com/watch?v=bilOPmugH4s`  ·  covers: Downtown Ault

Add to the description (near the top, above the hashtags):

```
📍 More of Ault the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/ault.html
```

### 309 views — Under $450,000 in Denver? This one's a Hidden Gem Near Garfield Lake Park
`youtube.com/watch?v=RenD0cRPD_k`  ·  covers: Garfield Lake Park

Add to the description (near the top, above the hashtags):

```
📍 More of Denver the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/denver/denver.html
```

### 279 views — Is this the best Lake in Fort Collins?
`youtube.com/watch?v=YvIPzWebofA`  ·  covers: Horsetooth Reservoir

Add to the description (near the top, above the hashtags):

```
📍 More of Fort Collins the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/fort-collins.html
```

### 260 views — Where Is Red Feather Lakes? Exploring Colorado's Hidden Gem & Gnome Road!
`youtube.com/watch?v=xn5fyEM7EkQ`  ·  covers: Gnome Road (Elf Lane)

Add to the description (near the top, above the hashtags):

```
📍 More of Red Feather Lakes the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/red-feather-lakes.html
```

### 156 views — Taste of Loveland 2026
`youtube.com/watch?v=wMtJCTePofE`  ·  covers: Taste of Loveland

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 78 views — Discover Beaver Meadows: Winter Magic with Elves and Fun Activities!
`youtube.com/watch?v=NoKEJ3uQQ0Q`  ·  covers: Beaver Meadows Resort

Add to the description (near the top, above the hashtags):

```
📍 More of Red Feather Lakes the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/red-feather-lakes.html
```

### 58 views — Why Loveland Buyers Love The Olde Course | Colorado Golf Living
`youtube.com/watch?v=Jz4kQHtpfzM`  ·  covers: The Olde Course at Loveland

Add to the description (near the top, above the hashtags):

```
📍 More of Loveland the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/larimer/loveland.html
```

### 21 views — What Makes a Home Luxurious? | Colliers Hill Erie CO Luxury Home
`youtube.com/watch?v=K2XYDr2cgYU`  ·  covers: Colliers Hill

Add to the description (near the top, above the hashtags):

```
📍 More of Erie the way locals actually see it — every spot I've filmed here,
plus homes for sale nearby: https://signaturepropertycollection.com/communities/weld/erie.html
```

---

## Your Google reviews

Google reviews can't carry a clickable link, so these work differently: mention the
site in the review text itself next time you post or edit one. Keep it natural — a
recommendation that reads like an advert gets ignored, and yours don't.

- **Cocina & Cantina** (10,000 review views) — town page: `https://signaturepropertycollection.com/communities/larimer/berthoud.html`
- **Dragon Inn** (5,300 review views) — town page: `https://signaturepropertycollection.com/communities/weld/eaton.html`
- **A&W Restaurant** (50 review views) — town page: `https://signaturepropertycollection.com/communities/weld/eaton.html`

Suggested phrasing to work in, not paste verbatim:

```
I map the local spots I actually eat at and hike around Northern Colorado —
signaturepropertycollection.com/communities
```

---

## The one link that works everywhere

Bio, pinned comment, Google Business profile, email signature:

```
https://signaturepropertycollection.com/communities
```

That is the map with all 35 spots on it. From there a visitor reaches every
town page, every video, and a live search of homes near any of them.

## And for sellers

When a seller asks why they should list with you:

```
https://signaturepropertycollection.com/seller-local-proof
```

They pick their town and see how many people have already watched or read your
content about it. No other agent in your market can show that number.
